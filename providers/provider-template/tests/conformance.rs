...
use gorce_provider_abi::{
    ...
    ScopedSecretDelivery, ToolInvokeParams, VersionRange, METHOD_INITIALIZE, METHOD_SHUTDOWN,
    METHOD_TOOL_INVOKE,
};
use provider_template::{
    archive_bytes_for_executable, API_KEY_CREDENTIAL_CLASS, API_KEY_METHOD_ID, OAUTH_METHOD_ID,
    PROVIDER_ID, SENTINEL_SECRET, TOOL_WEB_SEARCH,
};
...
fn spawned_package_conforms_to_canonical_v1_and_redacts_secret_from_results() {
    let executable = env!("CARGO_BIN_EXE_provider-template");
    ...
    let shutdown = raw_round_trip(
        &mut reader,
        &mut writer,
        &encode_message(&request("req-3", METHOD_SHUTDOWN, json!({}))).unwrap(),
    );
...
#[test]
fn manifest_is_valid_and_declares_both_auth_shapes() {
    let manifest = provider_template::manifest_for_executable(b"fixture-executable");
    manifest.validate().unwrap();
    assert!(manifest.auth_method(API_KEY_METHOD_ID).is_some());
    assert!(manifest.auth_method(OAUTH_METHOD_ID).is_some());
    let search = manifest.tool(TOOL_WEB_SEARCH).unwrap();
    assert_eq!(search.auth_method_id.as_deref(), Some(API_KEY_METHOD_ID));
    assert_eq!(search.network_origins, [provider_template::API_ORIGIN]);
}
